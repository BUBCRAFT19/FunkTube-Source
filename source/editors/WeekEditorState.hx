...
class WeekEditorState extends MusicBeatState
{
	...
	{
		var bgDefault:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFFF9CF51);
		...
		var topBarVis:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width, 120, FlxColor.BLACK);
		topBarVis.screenCenter(X);
		add(topBarVis);

		var topBarInvis:FlxSprite = new FlxSprite(topBarVis.x, topBarVis.y + 120).makeGraphic(FlxG.width, 10, FlxColor.BLACK);
		topBarInvis.screenCenter(X);
		topBarInvis.alpha = 0.5;
		add(topBarInvis);

		var bottomBarVis:FlxSprite = new FlxSprite(0, 600).makeGraphic(FlxG.width, 120, FlxColor.BLACK);
		bottomBarVis.screenCenter(X);
		add(bottomBarVis);

		var bottomBarInvis:FlxSprite = new FlxSprite(bottomBarVis.x, bottomBarVis.y - 15).makeGraphic(FlxG.width, 15, FlxColor.BLACK);
		...
		weekThing = new MenuItem(0, 620, weekFileName);
		weekThing.screenCenter(X);
		weekThing.antialiasing = ClientPrefs.globalAntialiasing;
		add(weekThing);

		lock = new FlxSprite(0, 620);
	...
	function addEditorBox()
	{
		...
		var loadWeekButton:FlxButton = new FlxButton(20, 650, "Load Week", function()
		{
			loadWeek();
		});
		add(loadWeekButton);

		var freeplayButton:FlxButton = new FlxButton(140, 650, "Freeplay", function()
		{
			MusicBeatState.switchState(new WeekEditorFreeplayState(weekFile));
		});
		add(freeplayButton);

		var saveWeekButton:FlxButton = new FlxButton(260, 650, "Save Week", function()
		{
			saveWeek(weekFile);
		});
		add(saveWeekButton);
